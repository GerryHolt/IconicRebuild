<div class="side-bar">
    <div class="side-item">
        <!-- Home Page - Premium [async] -->
        <script type="text/javascript">if (!window.AdButler) {
                (function () {
                    var s = document.createElement("script");
                    s.async = true;
                    s.type = "text/javascript";
                    s.src = 'https://servedbyadbutler.com/app.js';
                    var n = document.getElementsByTagName("script")[0];
                    n.parentNode.insertBefore(s, n);
                }());
            }</script>
        <script type="text/javascript">
            var AdButler = AdButler || {}; AdButler.ads = AdButler.ads || [];
            var abkw = window.abkw || '';
            var plc298339 = window.plc298339 || 0;
            document.write('<'+'div id="placement_298339_'+plc298339+'"></'+'div>');
            AdButler.ads.push({handler: function(opt){ AdButler.register(171978, 298339, [300,600], 'placement_298339_'+opt.place, opt); }, opt: { place: plc298339++, keywords: abkw, domain: 'servedbyadbutler.com', click:'CLICK_MACRO_PLACEHOLDER' }});
        </script>
    </div>
    <div class="side-item">
        <ul class="social-bar">
            <li class="icon-facebook icons"><a href="<?php the_field('facebook', 'options'); ?>" target="_blank"></a></li>
            <li class="icon-instagram icons"><a href="<?php the_field('instagram', 'options'); ?>" target="_blank"></a></li>
            <li class="icon-twitter icons"><a href="<?php the_field('twitter', 'options'); ?>" target="_blank"></a></li>
            <li class="icon-pinterest icons"><a href="<?php the_field('pinterest', 'options'); ?>" target="_blank"></a></li>
            <li class="icon-play icons"><a href="<?php the_field('youtube', 'options'); ?>" target="_blank"></a></li>
        </ul>
    </div>
    <div class="side-item">
        <h4>Subscribe to receive exclusive offers and the latest from</h4>
        <img src="/ui/images/logo-sm.png" />
        <?php echo do_shortcode( '[activecampaign form=5]' ); ?>
    </div>
    <div class="side-item why-us">
        <!-- ALL PAGES - IC Manifesto Ad -->
        <script type="text/javascript">if (!window.AdButler) {
                (function () {
                    var s = document.createElement("script");
                    s.async = true;
                    s.type = "text/javascript";
                    s.src = 'https://servedbyadbutler.com/app.js';
                    var n = document.getElementsByTagName("script")[0];
                    n.parentNode.insertBefore(s, n);
                }());
            }</script>
            <script type="text/javascript">
                var AdButler = AdButler || {}; AdButler.ads = AdButler.ads || [];
                var abkw = window.abkw || '';
                var plc311644 = window.plc311644 || 0;
                document.write('<'+'div id="placement_311644_'+plc311644+'"></'+'div>');
                AdButler.ads.push({handler: function(opt){ AdButler.register(171978, 311644, [300,300], 'placement_311644_'+opt.place, opt); }, opt: { place: plc311644++, keywords: abkw, domain: 'servedbyadbutler.com', click:'CLICK_MACRO_PLACEHOLDER' }});
            </script>
    </div>
    <div class="side-item">
    <!-- HOME PAGE - Foundation (Bottom Sidebar) -->
    <script type="text/javascript">if (!window.AdButler) {
                (function () {
                    var s = document.createElement("script");
                    s.async = true;
                    s.type = "text/javascript";
                    s.src = 'https://servedbyadbutler.com/app.js';
                    var n = document.getElementsByTagName("script")[0];
                    n.parentNode.insertBefore(s, n);
                }());
            }</script>
            <script type="text/javascript">
                var AdButler = AdButler || {}; AdButler.ads = AdButler.ads || [];
                var abkw = window.abkw || '';
                var plc309189 = window.plc309189 || 0;
                document.write('<'+'div id="placement_309189_'+plc309189+'"></'+'div>');
                AdButler.ads.push({handler: function(opt){ AdButler.register(171978, 309189, [300,600], 'placement_309189_'+opt.place, opt); }, opt: { place: plc309189++, keywords: abkw, domain: 'servedbyadbutler.com', click:'CLICK_MACRO_PLACEHOLDER' }});
            </script>
    </div>
</div>
